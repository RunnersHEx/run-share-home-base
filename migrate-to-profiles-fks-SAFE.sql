-- =====================================================
-- MIGRATE ALL FOREIGN KEYS FROM auth.users TO profiles (SAFE VERSION)
-- =====================================================
-- This version safely handles existing constraints
...
-- Step 2: BOOKINGS TABLE - Migrate to profiles (SAFE)
...
BEGIN
    ...
    -- Add foreign keys to profiles ONLY if they don't already exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'bookings' 
...
-- Step 3: PROPERTIES TABLE - Migrate to profiles (SAFE)
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'properties' 
        AND constraint_name = 'properties_owner_id_profiles_fkey'
    ) THEN
        ALTER TABLE public.properties 
        ADD CONSTRAINT properties_owner_id_profiles_fkey 
        FOREIGN KEY (owner_id) REFERENCES public.profiles(id) ON DELETE CASCADE;
        RAISE NOTICE 'Added properties_owner_id_profiles_fkey constraint';
    ELSE
        RAISE NOTICE 'properties_owner_id_profiles_fkey constraint already exists';
    END IF;
END $$;

SELECT 'Properties table updated successfully' as result;

-- Step 4: RACES TABLE - Migrate to profiles (SAFE)
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'races' 
        AND constraint_name = 'races_host_id_profiles_fkey'
    ) THEN
        ALTER TABLE public.races 
        ADD CONSTRAINT races_host_id_profiles_fkey 
        FOREIGN KEY (host_id) REFERENCES public.profiles(id) ON DELETE CASCADE;
        RAISE NOTICE 'Added races_host_id_profiles_fkey constraint';
    ELSE
        RAISE NOTICE 'races_host_id_profiles_fkey constraint already exists';
    END IF;
END $$;

SELECT 'Races table updated successfully' as result;

-- Step 5: BOOKING_MESSAGES TABLE - Migrate to profiles (SAFE)
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'booking_messages' 
        AND constraint_name = 'booking_messages_sender_id_profiles_fkey'
    ) THEN
        ALTER TABLE public.booking_messages 
        ADD CONSTRAINT booking_messages_sender_id_profiles_fkey 
        FOREIGN KEY (sender_id) REFERENCES public.profiles(id) ON DELETE CASCADE;
        RAISE NOTICE 'Added booking_messages_sender_id_profiles_fkey constraint';
    ELSE
        RAISE NOTICE 'booking_messages_sender_id_profiles_fkey constraint already exists';
    END IF;
END $$;

SELECT 'Booking messages table updated successfully' as result;

-- Step 6: POINTS_TRANSACTIONS TABLE - Migrate to profiles (SAFE)
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'points_transactions' 
        AND constraint_name = 'points_transactions_user_id_profiles_fkey'
    ) THEN
        ALTER TABLE public.points_transactions 
        ADD CONSTRAINT points_transactions_user_id_profiles_fkey 
        FOREIGN KEY (user_id) REFERENCES public.profiles(id) ON DELETE CASCADE;
        RAISE NOTICE 'Added points_transactions_user_id_profiles_fkey constraint';
    ELSE
        RAISE NOTICE 'points_transactions_user_id_profiles_fkey constraint already exists';
    END IF;
END $$;

SELECT 'Points transactions table updated successfully' as result;

-- Step 7: OTHER TABLES - Check and update if they exist (SAFE)
...
BEGIN
    IF EXISTS (
        ...
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
            WHERE table_name = 'verification_requests' 
            AND constraint_name = 'verification_requests_user_id_profiles_fkey'
        ) THEN
            ALTER TABLE public.verification_requests 
            ADD CONSTRAINT verification_requests_user_id_profiles_fkey 
            FOREIGN KEY (user_id) REFERENCES public.profiles(id) ON DELETE CASCADE;
            RAISE NOTICE 'Added verification_requests_user_id_profiles_fkey constraint';
        ELSE
            RAISE NOTICE 'verification_requests_user_id_profiles_fkey constraint already exists';
        END IF;
    END IF;
END $$;
    ...
    IF EXISTS (
        ...
        IF EXISTS (
            ...
            IF NOT EXISTS (
                SELECT 1 FROM information_schema.table_constraints 
                WHERE table_name = 'reviews' 
                AND constraint_name = 'reviews_reviewer_id_profiles_fkey'
            ) THEN
                ...
                RAISE NOTICE 'Added reviews_reviewer_id_profiles_fkey constraint';
            ELSE
                RAISE NOTICE 'reviews_reviewer_id_profiles_fkey constraint already exists';
            END IF;
        END IF;
        ...
        IF EXISTS (
            ...
            IF NOT EXISTS (
                SELECT 1 FROM information_schema.table_constraints 
                WHERE table_name = 'reviews' 
                AND constraint_name = 'reviews_reviewee_id_profiles_fkey'
            ) THEN
                ALTER TABLE public.reviews 
                ADD CONSTRAINT reviews_reviewee_id_profiles_fkey 
                FOREIGN KEY (reviewee_id) REFERENCES public.profiles(id) ON DELETE CASCADE;
                RAISE NOTICE 'Added reviews_reviewee_id_profiles_fkey constraint';
            ELSE
                RAISE NOTICE 'reviews_reviewee_id_profiles_fkey constraint already exists';
            END IF;
        END IF;
    END IF;
END $$;
...
BEGIN
    IF EXISTS (
        ...
        IF NOT EXISTS (
            ...
            FOREIGN KEY (user_id) REFERENCES public.profiles(id) ON DELETE CASCADE;
            RAISE NOTICE 'Added admin_users_user_id_profiles_fkey constraint';
        ELSE
            RAISE NOTICE 'admin_users_user_id_profiles_fkey constraint already exists';
        END IF;
    END IF;
END $$;
