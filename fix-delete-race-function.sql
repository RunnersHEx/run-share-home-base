-- FIX FOR ADMIN_DELETE_RACE FUNCTION
-- This replaces the existing function with better error handling and RLS bypass
...
BEGIN
  -- Validate inputs
  IF race_id IS NULL OR admin_user_id IS NULL THEN
  ...
  -- Delete property availability records if they reference this race
  -- (This might not exist but just in case)
  ...
  RETURN jsonb_build_object(
    ...
    'deleted_bookings', (SELECT COUNT(*) FROM bookings WHERE race_id = race_id AND status = 'cancelled')
  );
...
-- Add RLS bypass policies specifically for admin delete operations
DO $$
BEGIN
  -- Ensure admins can delete races
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'races' AND policyname = 'Admins can delete races'
  ) THEN
    CREATE POLICY "Admins can delete races" 
      ON public.races 
      FOR DELETE 
      USING (true); -- Will be secured by the function itself
  END IF;
  
  -- Ensure admins can delete race images
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'race_images' AND policyname = 'Admins can delete race images'
  ) THEN
    CREATE POLICY "Admins can delete race images" 
      ON public.race_images 
      FOR DELETE 
      USING (true); -- Will be secured by the function itself
  END IF;
END
...
-- Test the function by running this (replace with actual UUIDs when testing):
-- SELECT admin_delete_race('your-race-id-here', 'your-admin-user-id-here', 'Test deletion');
