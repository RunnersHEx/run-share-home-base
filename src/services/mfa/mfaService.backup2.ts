...
/**
 * Send MFA code via EmailJS (real email sending)
 */
async function sendMFACodeEmail(email: string, code: string): Promise<boolean> {
  try {
    // Check if EmailJS is configured
    if (!EMAILJS_SERVICE_ID || !EMAILJS_TEMPLATE_ID || !EMAILJS_PUBLIC_KEY) {
      logger.warn('EmailJS not configured, falling back to console display');
      return false; // Fall back to console display
    }
    ...
    logger.info('Sending MFA email via EmailJS to:', email);
    ...
    if (result.status === 200) {
      logger.info('MFA email sent successfully via EmailJS:', result);
      return true;
    } else {
...
/**
 * Send MFA code via mailto (fallback method)
 */
async function sendMFACodeMailto(email: string, code: string): Promise<boolean> {
  try {
    ...
    const body = encodeURIComponent(
      ...
      `${code}\n\n` +
      `Este código expira en ${MFA_CODE_EXPIRY_MINUTES} minutos.\n\n` +
    ...
    try {
      ...
      logger.warn('Mailto failed:', error);
      return false;
    }
    
  } catch (error) {
    logger.error('Failed to send mailto:', error);
    return false;
  }
}
