...
-- Migration: Fixes constraint problems and parameter mismatches
-- =============================================

-- Step 1: Drop all race_date constraints that interfere with status updates
DO $$
DECLARE
    constraint_rec RECORD;
BEGIN
    -- Drop known problematic constraints
...
-- Step 2: Create smart validation function that doesn't interfere with status updates
...
BEGIN
    ...
    ELSIF TG_OP = 'UPDATE' THEN
        -- Only check if race_date column is being changed
...
-- Step 3: Create trigger for smart validation
...
-- Step 4: Fix admin_update_race_status function with correct parameter names
...
BEGIN
  ...
  -- Update ONLY the is_active and updated_at fields
  UPDATE races 
...
-- Step 5: Fix admin_delete_race function with correct parameter names
...
-- Add comments
COMMENT ON FUNCTION validate_race_date_smart() IS 'Smart validation that only checks race_date when inserting or when race_date is being updated';
...
COMMENT ON FUNCTION admin_update_race_status IS 'Updates race status with correct parameter names - Fixed constraint and parameter issues';
COMMENT ON FUNCTION admin_delete_race IS 'Deletes race with correct parameter names - Consistent with frontend expectations';
